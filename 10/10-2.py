...
def dfs(visited, graph, node):  #function for dfs 
    if node not in visited:
        print (node, end=" ")
        print (len(check_adjacent(txt,node))==1)
        visited.add(node)
...
if __name__ == "__main__":
    ...
    total = 0
    for zero in find_zeros(txt)[0:1]:
        visited = []
        queue =[]
        ...
        print(value_list)
        result_list = list(filter(lambda x: x not in ['0', '9'], value_list))
